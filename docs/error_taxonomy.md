...
## Per-Dataset Error Profile (Gemma 4 26B via Gemini API)

Generated by `scripts/error_analysis.py` (Fase 3).

| Pipeline | E01 | E02 | E03 | E04 | E05 | E06 | E07 | LLM_ERR | CORRECT_REF |
|----------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:-------:|:-----------:|
| B — Markdown | 6 | 0 | 1 | 8 | 1 | **23** | 0 | 3 | 8 |
| A — Raw text | 4 | 0 | 4 | 9 | 0 | **21** | 0 | 2 | 10 |

**Key findings:**
- **E06 (false_refusal)** is the dominant error mode (42-46% of questions) — Gemma 4 often says "cannot find information" even when the correct chunk was retrieved.
- **E04 (citation_missing)** is the second most common (16-18%) — the model provides correct answers but fails to cite the source document.
- Pipeline A (Raw) has fewer retrieval misses (4 vs 6) but more context_ignored cases (4 vs 1).
- Pipeline A has more correct refusals on negative questions (10 vs 8), suggesting raw text helps identify unanswerable questions.
- Oracle tests confirm that ~67-85% of errors are retrieval-related (insufficient context from TOP_K=3), not true generation failures.
