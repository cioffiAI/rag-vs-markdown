...
\begin{abstract}
Retrieval-Augmented Generation (RAG) pipelines typically embed raw document text into a vector database. An alternative---preprocessing documents into structured Markdown before indexing---promises better chunk coherence, preserved table structure, and clearer section boundaries. We benchmark both strategies across four model-provider configurations (0.8B to 26B parameters) on a 50-question corpus of 10 arXiv papers. On Qwen~0.8B (LM Studio), Markdown yields a +1.2\% advantage. For the three larger configurations (Nemotron~3, DeepSeek~V4~Flash, Gemma~4~26B across two providers), raw text outperforms Markdown ($-$3.6\% to $-$5.2\%). This reversal correlates with model capability but is confounded by provider identity. We identify a mechanism: 25.3\% of retrieved Markdown chunks are \emph{shallow}---section headers and table markup with $\leq$200 informative characters---that compete with substantive chunks via cosine similarity, producing embedding false positives. An oracle test reveals an +85\% retrieval ceiling. These findings show that document preprocessing strategy interacts with the generator configuration in a non-trivial way and cannot be assumed universal.
\end{abstract}
...
Our central finding is that the preprocessing advantage is not universal: it inverts across the four model-provider configurations we tested. The smallest model (Qwen~0.8B) benefits from Markdown; all larger models perform better with raw text. We argue this pattern is driven by a retrieval failure mode we call \emph{shallow chunks}: structurally rich but informationally empty Markdown fragments that capture embedding similarity via header keywords while contributing no factual content. An oracle test that bypasses retrieval entirely confirms that retrieval quality---not generation quality---dominates the performance gap, though the test has diagnostic limitations we discuss.

The paper makes three contributions:

\begin{enumerate}
    \item A 4-configuration, 2-pipeline benchmark revealing a \textbf{model $\times$ pipeline interaction}: the Markdown advantage erodes and reverses across the four configurations tested.
...
The Markdown advantage (Qwen) becomes a Markdown disadvantage for all larger models, with the gap widening monotonically: $-$3.6\% (Nemotron 3), $-$4.4\% (DeepSeek V4 Flash), $-$5.2\% (Gemma 4 26B). This gradient is consistent across all four configurations tested, but model size, provider, and API co-vary---we report it as an observed pattern, not a controlled experiment on model capability.
...
\begin{table}[h]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}p{3.8cm}>{\raggedright\arraybackslash}p{7cm}>{\raggedright\arraybackslash}p{2.5cm}}
        ...
        & Supported in our setup ($\Delta$ $-$3.6\% to $-$5.2\% for 3B+ models)
        & Moderate \\
        ...
        & Strong \\
        Larger models prefer raw text
        & Observed across 4 configs but confounded by provider, API, prompt
        & Weak --- needs controlled family test \\
        Markdown is harmful in general
        & Not demonstrated
        & Do not claim \\
        Filtering shallow chunks would recover Markdown's position
        & Hypothesis only, not yet tested (Pipeline C planned)
        & Untested \\
        \bottomrule
    \end{tabular}
\end{table}
